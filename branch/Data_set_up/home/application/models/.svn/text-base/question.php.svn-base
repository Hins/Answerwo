...
class application_models_question extends Zend_Db_Table{
    ...
    public function retrieve($keyword,$createTime,$lastmodifyTime,$source,$type,$category,$level,$difficulty) {
        $db = application_common_dbutil::getDb();
        $select = $db->select();
        $select->from($this->_name, array('id','createTime','lastmodifyTime','type','level','content','source','category','answer','difficulty','encryptid'));
        if($createTime != ""){
            $select->where("createTime > ?", "$createTime");
        }
    	if($lastmodifyTime != ""){
            $select->where("lastmodifyTime > ?", "$lastmodifyTime");
        }
    	if($source != ""){
            $select->where("source=?", intval($source));
        }
    	if($type != ""){
            $select->where("type=?", intval($type));
        }
    	if($category != ""){
            $select->where("category=?", intval($category));
        }
    	if($level != ""){
        ...
        if($keyword != ""){
        	$select->where("content like ?", "%$keyword%");
        }
        $sql = $select->__toString();
        $result = $db->query($sql);
    ...
    public function retrieveWithNumber($keyword,$createTime,$lastmodifyTime,$source,$type,$category,$level,$difficulty,$page,$number) {
    	$db = application_common_dbutil::getDb();
    	$select = $db->select();
    	$select->from($this->_name, array('id','createTime','lastmodifyTime','type','level','content','source','category','answer','difficulty','encryptid'));
    	if($createTime != ""){
