...
class application_models_question extends Zend_Db_Table{
	...
	public function GetQuestionsByNumber($level,$type,$number) {
    	$db = application_common_dbutil::getDb();
    	$select = $db->select();
    	$select->from($this->_name, array('id','content','answer'));
    	if($level == "2/3"){
    		$select->where("level = ?", 2);
    		//$select->orWhere('level = ?', 3);
    	}
