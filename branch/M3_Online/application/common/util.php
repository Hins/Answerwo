...
class application_common_util {
	...
	public static function Log()
	{
		...
		fwrite($file_pointer, application_common_util::curPageURL()."	".application_common_util::getReferrer().application_common_util::getIp()."	".date("Y-m-d H:i:s")."	".application_common_userutil::getCurUser()."\n");
		fclose($file_pointer);  
	}
	
	public static function IPFilter()
	{
		/*if(application_common_util::getIp() == "66.249.67.112")
		{
			return true;
		}*/
		return false;
	...
	{	
		$frontendOpts = array( 
							'caching' => true, 
							'lifetime' => 3600, //缓存生命周期60分钟，根据自己项目需求设置
	...
	public static function GetCacheInstance()
	{
		if (Zend_Registry::isRegistered("memcache")) { 
			return Zend_Registry::get("memcache"); 
		}
		application_common_util::CreateCache();
		return Zend_Registry::get('memcache');
	}
	
	public static function SetCache($id,$val)
	{
		$memcache = application_common_util::GetCacheInstance();
       	$tags = array();
      	$lifetime = 3600;
      	$memcache->save($val,$id,$tags,$lifetime);
	}
	
	public static function GetCache($id)
	{
		$memcache = application_common_util::GetCacheInstance();
		return $memcache->load($id);
	}
	
	public static function GetRandom($upperbound)
	{
