    	...
    	if(isset($_GET["user"]) && $_GET["user"] != "" && !application_common_util::inject_check($_GET["user"])) {
			...
			if($qids != false) {
				...
				foreach($arrfavorite as $val) {
					$arrfavorite[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$val["encryptid"];
			...
			if($arrexplain) {
				...
				foreach($arrexplain as $val) {
					...
					$arrexplain[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$arrs["encryptid"];
			...
			if($arrdiscuss) {
				...
				foreach($arrdiscuss as $val) {
					...
					$arrdiscuss[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$arrs["encryptid"];
			...
			if($arrrecord) {
				...
				foreach($arrrecord as $val) {
					...
					$arrrecord[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$arrs["encryptid"];
    	...
    	if(isset($_GET["user"]) && $_GET["user"] != "" && !application_common_util::inject_check($_GET["user"])) {
			...
			if($arr) {
				...
				foreach($arr as $val) {
					...
					$arr[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$arrs["encryptid"];
    ...
    public function practiceAction() {
    	...
    	if(isset($_GET["user"]) && $_GET["user"] != "" && !application_common_util::inject_check($_GET["user"])) {
			...
			foreach($encryptids as $encryptid) {
				...
				$encryptids[$i]["link"] = WEB_DOMAIN."/p/practiceset?encryptid=".$encryptid["encryptid"];
				$i++;
			}
    ...
    public function practicesetAction() {
    	...
    	if(isset($_GET["encryptid"]) && $_GET["encryptid"] != "" && !application_common_util::inject_check($_GET["encryptid"])) {
			...
			foreach($questions as $question) {
				...
				$arr["link"] = WEB_DOMAIN."/q/detail?encryptid=".$arr["encryptid"];
