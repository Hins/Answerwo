...
class application_models_discuss extends Zend_Db_Table {
    ...
    public function getAllDiscussByuid($uid) {
    	$db = application_common_dbutil::getDb();
    	$select = $db->select();
    	$select->from('discuss', array('id','createTime','question_id','content'));
    ...
    public function getDiscussByUidandTime($uid,$timestamp) {
    	$db = application_common_dbutil::getDb();
        $select = $db->select();
        $select->from($this->_name, array('question_id'));
        $dateexpression = "current_timestamp() - ".$timestamp;
    	$select->where("createTime >= ?", new Zend_Db_Expr($dateexpression));
    	$select->where('user_id=?',intval($uid));
        $sql = $select->__toString();
        $result = $db->query($sql);
