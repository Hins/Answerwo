...
class UController extends Zend_Controller_Action {
	...
	public function addquestionAction()
    {
		...
		if($user != false) {
			...
			if(intval($arr["role"] != 2)) {
				header("Location: http://prepublish.answerwo.com");
			}
		}
    ...
    public function editquestionAction()
    {
		...
		if($user != false) {
			...
			$arr = $obj_user->getUserByName($user);
			if(intval($arr["role"] != 2)) {
				header("Location: http://prepublsih.answerwo.com");
			}
		}
    ...
    public function listquestionAction() {
    	...
    	try{
    		...
    		for($i=0;$i<5 && $page+$i <= intval($this->view->pagenumber);$i++) {
    			$subpage = "<a href=\"http://prepublish.answerwo.com/u/listquestion?page=".($page+$i)."\" >".($page+$i)."</a> ";
    			$pagination .= $subpage;
    		}
    ...
    public function discussAction() {
    	...
    	try{
    		...
    		$pagination = "";
    		for($i=0;$i<5 && $page+$i <= intval($this->view->pagenumber);$i++) {
    			$subpage = "<a href=\"http://prepublish.answerwo.com/u/discuss?page=".($page+$i)."\" >".($page+$i)."</a> ";
    			$pagination .= $subpage;
    		}
    		$this->view->pagination = $pagination;
    	}
    ...
    public function userlistAction() {
    	...
    	try{
    		...
    		$pagination = "";
    		for($i=0;$i<5 && $page+$i <= intval($this->view->pagenumber);$i++) {
    			$subpage = "<a href=\"http://prepublish.answerwo.com/u/userlist?page=".($page+$i)."\" >".($page+$i)."</a> ";
    			$pagination .= $subpage;
    		}
    		$this->view->pagination = $pagination;
    	}
    	...
    	if(isset($_POST["nickname"]) && $_POST["nickname"] != "") {
    		try {
    			...
    			if(!application_common_userutil::getUserByNameAndEmail($nickname,$email)) {
    				...
    				echo "<meta http-equiv=\"refresh\" content=\"1;url=http://prepublish.answerwo.com/s/successregister\">";
    			}
    			else {
    	...
    	if(isset($_GET["name"]) && $_GET["name"] != "") {
    		try {
    			...
    			if(($row=application_common_userutil::getUserByNameOrEmail($name,$password)) != false) {
    				...
    				echo "<meta http-equiv=\"refresh\" content=\"1;url=http://prepublish.answerwo.com/s/successlogin\">";
    			}
    			else {
