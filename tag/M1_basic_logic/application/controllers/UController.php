...
class UController extends Zend_Controller_Action {
    	...
    	if(isset($_POST["content"]) && $_POST["content"] != "") {
    		try {
    			if((isset($_POST["content"]) && application_common_util::inject_check($_POST["content"]))
    				...
    				|| (isset($_POST["knowledge"]) && application_common_util::inject_check($_POST["knowledge"]))
    			...
    			$arr = array(
    						...
    						"level" => intval(isset($_POST["source"]) ? application_common_util::GetLevel($_POST["source"]) : "1"),
    						"content" => $_POST["content"],
    						"source" => intval(isset($_POST["source"]) ? application_common_util::GetSource($_POST["source"]) : "0"),
    						"category" => intval(isset($_POST["knowledge"]) ? application_common_util::KnowMap($_POST["knowledge"]) : "0"),
    ...
    public function editquestionAction()
    {
    	...
    	if(isset($_GET["search"]) && $_GET["search"] != "") {
    		try {
    			...
    			$this->view->knowledge = application_common_util::RevKnowMap($arr["category"]);
    	...
    	if(isset($_GET["id"]) && $_GET["id"] != "") {
    		try {
    			if((isset($_GET["content"]) && application_common_util::inject_check($_GET["content"]))
    				|| (isset($_GET["answer"]) && application_common_util::inject_check($_GET["answer"]))
    				|| (isset($_GET["id"]) && application_common_util::inject_check($_GET["id"]))
    				|| (isset($_GET["knowledge"]) && application_common_util::inject_check($_GET["knowledge"]))
    				...
    				|| (isset($_GET["answer1"]) && application_common_util::inject_check($_GET["answer1"]))
    				|| (isset($_GET["answer2"]) && application_common_util::inject_check($_GET["answer2"]))
    				|| (isset($_GET["answer3"]) && application_common_util::inject_check($_GET["answer3"]))
    				|| (isset($_GET["answer4"]) && application_common_util::inject_check($_GET["answer4"]))
    				|| (isset($_GET["explain"]) && application_common_util::inject_check($_GET["explain"]))) {
    			...
    			if($arr["category"] != application_common_util::KnowMap($_GET["knowledge"])) {
    				$obj_question->UpdataCategory($qid,application_common_util::KnowMap($_GET["knowledge"]));
    			}
    ...
    public function listquestionAction() {
    	...
    	try{
    		...
    		$len = count($arr);
    		for($i=0;$i<$len;$i++) {
    			$arr[$i]["category"] = application_common_util::RevKnowMap($arr[$i]["category"]);
    ...
    public function searchquestionAction() {
    	...
    	if(isset($_GET["content"])) {
    		try {
    			...
    			$obj_question = new application_models_question();
    			$arr = $obj_question->retrieve($_GET["content"],$_GET["createTime"],$_GET["lastmodifyTime"],$_GET["source"],$_GET["type"],$_GET["category"],$_GET["level"],$_GET["difficulty"]);
    			$this->view->questions = Zend_Json::encode($arr);
    		}
    		catch(Zend_Exception $e){
    			echo $e->getMessage()."<br>";
    	...
    	if(isset($_GET["name"]) && $_GET["name"] != "") {
    		try {
    			...
    			if(($row=application_common_userutil::getUserByNameOrEmail($name,$password)) != false) {
    				application_common_userutil::setLoginCookie($row["name"], application_common_constant::COOKIE_USER_UNIQUE_EXPIRE_SECONDS);
