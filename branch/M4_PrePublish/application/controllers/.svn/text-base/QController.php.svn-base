...
class QController extends Zend_Controller_Action {
	...
	public function detailAction() {
		...
		if(application_common_FrontDoor::Filter($ip)) {
			header('Location: http://prepublish.answerwo.com/s/robot');
		}
		//modify some content;
	...
	public function searchAction() {
    	...
    	application_common_FrontDoor::Check($ip);
		if(application_common_FrontDoor::Filter($ip)) {
			header('Location: http://prepublsih.answerwo.com/s/robot');
		}
		try {
			//get current user information;
			$curuser = application_common_userutil::getCurUser();
			...
			$i = 0;
			foreach($arr as $val) {
				$arr[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$val["encryptid"];
				$i++;
			}
			...
			if(isset($_GET["query"]) && $_GET["query"] != "") {
				...
				for($i=0;$i+$page < $pagenumber && $i < 10; $i++) {
					$pagecontext = "<a href=\"http://prepublsih.answerwo.com/q/search?query=".$_GET['query']."&page=".($i+$page)."\">".($i+$page)."</a> ";
					$this->view->pagination .= $pagecontext;
				}
				
				$this->view->lastpagination = "<a href=\"http://prepublish.answerwo.com/q/search?query=".$_GET['query']."&page=".$pagenumber."\">".$pagenumber."</a>";
	...
	public function listAction() {
		...
		try {
			...
			if($arr != false) {
				...
				for($i=0;$i+$page < $pagenumber && $i < 10; $i++) {
					$pagecontext = "<a href=\"http://prepublish.answerwo.com/q/list?&source=".$source."&type=".$type."&difficulty=".$difficulty."&level=".$level."&page=".($i+$page)."\">".($i+$page)."</a> ";
					$this->view->pagination .= $pagecontext;
				}
				if($page < $pagenumber) {
					$this->view->lastpaflag = 1;
					$this->view->lastpagination = "<a href=\"http://prepublish.answerwo.com/q/list?&source=".$source."&type=".$type."&difficulty=".$difficulty."&level=".$level."&page=".$pagenumber."\">".$pagenumber."</a>";
				}
	...
	public function reportAction() {
		...
		foreach($score_arr as $answer) {
			...
			$randomquestions[$i]["link"] = WEB_DOMAIN."/q/detail?encryptid=".$temp_arr["encryptid"];
		...
		$weekquestions = $obj_record->getRecordByUidandTime($curuser,4233600);
		$this->view->OneWeekTotal = ($weekquestions == 0 ? 0 : count($weekquestions));
		$correct = 0;
		foreach($weekquestions as $question) {
			$singlequestion = $obj_question->getQuestionById($question["question_id"]);
			if($singlequestion["answer"] == $question["answer"]) {
				$correct++;
			}
		}
		$this->view->OneWeekTotalPerc = intval((intval($correct) / count($weekquestions)) * 100);
		
		$weekdiscuss = $obj_discuss->getDiscussByUidandTime($curuser,4233600);
		$this->view->OneWeekDiscuss = ($weekdiscuss == 0 ? 0 : count($weekdiscuss));
		
		$questions = $obj_record->getAnswersByUid($curuser);
		$this->view->AllTotal = ($questions == false ? 0 : count($questions));
		$explains = $obj_explains->getExplainsByUid($curuser);
		$this->view->AllExplains = ($explains == false ? 0 : count($explains));
		$discusses = $obj_discuss->getAllDiscussByuid($curuser);
		$this->view->AllDiscusses = ($discusses == false ? 0 : count($discusses));
	}
}
?>
